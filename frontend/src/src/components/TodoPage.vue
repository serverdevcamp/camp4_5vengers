...
<div class="container">
  ...
  <ul class="list-group">
    <li class="list-group-item" v-for="todo in todos" v-bind:key="todo.name">
      {{todo.name}}
...
export default {
  methods: {
    ...
    deleteTodo (todo) {
      this.todos.forEach(function (_todo, i, obj) { // value, index, array
        if (_todo.id === todo.id) {
          this.$http.delete('http://todos.garam.xyz/api/todos/' + todo.id)
            .then((result) => {
              obj.splice(i, 1)
            })
        }
  ...
  mounted () { // Vue Component가 페이지에 끼워지고나서 호출되는 함수
